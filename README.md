...
- **Profile System**: Save and load multiple configuration profiles
- **Status Display**: Toggle visibility of status messages
- **Startup Options**: Option to run minimized to system tray

### Triggers
- **Manual Trigger Control**: Adjust LT/RT triggers with keyboard hotkeys
- **Trigger Intensity**: Adjustable step size (0.01 - 0.5) for trigger increment/decrement
- **Separate Trigger Mode**: Control LT and RT independently with dedicated hotkeys (S/A for LT, W/D for RT)
...
| LT + | `S` | Increase left trigger (separate mode) |
| LT − | `A` | Decrease left trigger (separate mode) |
| RT + | `W` | Increase right trigger (separate mode) |
| RT − | `D` | Decrease right trigger (separate mode) |
| Reset Triggers | `Alt+T` | Reset both triggers to 0 |
| Toggle Crosshair | `Alt+N` | Show/hide crosshair overlay |
| Toggle Trigger Overlay | `Alt+M` | Show/hide trigger overlay |
...
├── app/
│   ├── constants.py        # Win32 constants and VK codes
│   ├── config/            # Configuration management
│   │   ├── manager.py     # Config file I/O
│   │   ├── models.py      # AppConfig, HotkeyConfig, Theme dataclasses
│   │   └── profiles.py    # Default configurations
│   ├── controller/        # Virtual gamepad logic
│   │   ├── mapper.py      # ControllerMapper with thread-safe operations
│   │   └── math_utils.py  # Pure functions for stick/trigger calculations
│   ├── input/             # Keyboard input handling
│   │   ├── keyboard_hook.py  # Low-level Windows keyboard hook
│   │   ├── models.py      # Hotkey dataclasses
│   │   └── parser.py      # Hotkey string parsing
│   ├── overlays/          # Always-on-top overlays
│   │   ├── crosshair.py   # Crosshair overlay window
│   │   └── trigger.py     # Trigger bars overlay window
│   ├── system/            # System integration
│   │   ├── paths.py       # Path resolution utilities
│   │   └── tray.py        # System tray icon
│   └── gui/               # User interface
│       ├── app.py         # Main GUI coordinator (VctrlApp)
...
```json
{
  ...
  "hotkey_crosshair": "alt+n",
  "hotkey_trigger_overlay": "alt+m",
  "theme": "light",
  "crosshair": false,
  "trigger_overlay": false,
  "separate_triggers": false,
  "reset_opposite_trigger": false,
  "run_minimized": false,
  "trigger_intensity": 0.1,
  "show_status": true,
...
- **Architecture**: Modular Python application with clear separation of concerns
